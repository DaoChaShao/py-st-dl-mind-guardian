...
**MindGuardian** 是一个由 **AI 驱动的平台**，旨在帮助学生进行潜在心理健康风险的早期识别。

该平台结合 **深度学习（Deep Learning）** 进行风险预测，并利用 **大型语言模型（Large Language Model, LLM）** 提供
**个性化解释（Personalised Explanations）**、**支持性指导（Supportive Guidance）** 以及 **交互式心理咨询（Interactive
Counselling）**。

该平台通过 **数据驱动（Data-driven）** 的分析方法，帮助用户更好地理解影响心理风险的相关因素，同时提供直观的界面用于风险评估和人机交互。

MindGuardian 展示了 **预测分析（Predictive Analytics）**、**可解释人工智能（Explainable AI）** 和 **生成式人工智能（Generative
AI）** 的融合，旨在构建一个面向学生心理健康的实用型 **智能决策支持系统（Decision-Support System）**。

**数据描述**
---

该 **[数据集](https://www.kaggle.com/datasets/programmer3/student-psychological-crisis-risk-dataset)**
包含用于心理危机风险预测的模拟学生数据记录。

数据集涵盖多个类别的特征，包括心理指标、行为模式、生理信号以及学业表现指标。具体信息包括压力水平（Stress）、焦虑程度（Anxiety）、抑郁水平（Depression）、睡眠时长（Sleep
Duration）、屏幕使用时间（Screen Time）、社交媒体使用情况（Social Media Usage）、出勤率（Attendance）以及作业完成情况（Assignment
Performance）等因素。

每条数据记录均对应一个心理危机风险标签（Crisis Risk Label），可用于构建机器学习模型，实现心理风险的早期识别、可解释性分析，以及基于
AI 的心理健康辅助支持系统。
